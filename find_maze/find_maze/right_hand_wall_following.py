...
class GlobalLidarController(Node):
    def __init__(self):
        ...
        )  # SensorDataQoS()는 RELIABILITY=BestEffort, HISTORY=KeepLast로 맞춰져 있어 LiDAR와 잘 붙음.
        self.create_subscription(
    ...
    def decision_loop(self):
        ...
        self.get_logger().info(
            f"판단: front={self.front_mean:.2f}, right={self.right_mean:.2f}"
        )

        print(f"elf.right_mean{self.right_mean}")
        if self.right_mean > 0.40:
            self.get_logger().info("오른쪽 회전 요청")
            self.send_move_command(degree=-90.0, distance=0.4)
            self.FORWARD = True
        elif self.front_mean > 0.38:
            self.get_logger().info("⬆전진 요청")
            self.send_move_command(degree=0.0, distance=0.4)
        else:
            self.get_logger().info("⬅왼쪽 회전 요청")
