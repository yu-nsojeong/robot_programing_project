...
class CellMapGenerator:
    ...
    # 원점 위치 시각화
    def draw_origin(self, output_file):
        img_color = cv2.cvtColor(self.img, cv2.COLOR_GRAY2BGR)
        cv2.circle(img_color, (self.start_x, self.start_y), 1, (0, 0, 255), -1)
...
def extract_wall_info_from_scan_with_yaw(scan_msg, imu_yaw_degree):
    ...
    #     print(f"{i} : {ranges[i]:.2f}") # Added .2f for cleaner output
    ...
    for cell_dir, robot_angle in direction_map.items():
        ...
        #print(f"세부 거리 데이터 (dists): {dists}") # 필요하다면 dists 리스트 자체도 출력 가능
        result[cell_dir] = np.mean(dists) >= threshold if dists else False
        # print(f"@@@@cell : {cell_dir} index : {index} (벽 판단: {result[cell_dir]})")
        # print(f"@@@@imu_yaw_degree : {imu_yaw_degree} ")
